...
def generate_classification_report_and_roc_curve(true_labels, pred_probs, num_classes, output_dir='output'):
    ...
    with open(os.path.join(output_dir, 'classification_report_mobilenet_large.txt'), 'w') as f:
    ...
    plt.title('Receiver Operating Characteristic (ROC)')
    plt.legend(loc='lower right')
    plt.savefig(f'{output_dir}/roc_curve_mobilenetv3_large.png')
    plt.show()
...
learning_rate = 1e-4
num_classes = 5 
model_save_path = 'best_mobilenetv3_large_model.pth'
output_dir = 'output'
