...
# # CSV 파일에서 데이터 로드
# csv_file = 'sampled_image_data.csv'
...
# # 모델 정의 (MobileNetV3 Large)
# class MobileNetV3WithSize(nn.Module):
#     def __init__(self, num_classes=5):
#         super(MobileNetV3WithSize, self).__init__()
...
#     print("\nClassification Report:")
#     print(classification_report(true_labels, np.argmax(pred_probs, axis=1), target_names=[f'class_{i}' for i in range(num_classes)]))
...
# def train_and_evaluate(num_epochs=10, batch_size=32, learning_rate=1e-4, num_classes=5, model_save_path='best_model.pth', output_dir='output'):
#     # 데이터 로드 및 전처리 설정
...
#     model = MobileNetV3WithSize(num_classes=num_classes).to(device)
...
# output_dir='output'
# # 학습 실행
# train_and_evaluate(num_epochs=num_epochs, batch_size=batch_size, learning_rate=learning_rate, num_classes=num_classes, model_save_path=model_save_path, output_dir=output_dir)
...
class MobileNetV3WithSize(nn.Module):
    def __init__(self, num_classes=5):
        super(MobileNetV3WithSize, self).__init__()
        self.model = mobilenet_v3_large(pretrained=True)
        in_features = self.model.classifier[0].in_features  # MobileNetV3 Large의 입력 특징 수
...
def generate_classification_report_and_roc_curve(true_labels, pred_probs, num_classes, output_dir='output'):
    ...
    with open(os.path.join(output_dir, 'classification_report_mobilenet_large_sample.txt'), 'w') as f:
    ...
    plt.title('Receiver Operating Characteristic (ROC)')
    plt.legend(loc='lower right')
    plt.savefig(f'{output_dir}/roc_curve_mobilenetv3_large_sample.png')
    plt.show()
...
learning_rate = 1e-4
num_classes = 5 
model_save_path = 'best_mobilenetv3_large_model_sample.pth'
output_dir = 'output'
